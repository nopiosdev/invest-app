        ...
        private class AuthorizeCustomerFilter : IAsyncAuthorizationFilter
        {
            ...
            private async Task AuthorizeCustomerAsync(AuthorizationFilterContext context)
            {
                ...
                if (context.Filters.Any(filter => filter is AuthorizeCustomerFilter))
                {
                    ...
                    else if ((context?.ActionDescriptor?.RouteValues["controller"]?.Equals("Home") ?? false) &&
                        (context?.ActionDescriptor?.RouteValues["action"]?.Equals("Index") ?? false) &&
                        (context?.ActionDescriptor?.RouteValues["area"] == null))
                    {
                        if (customer.Verified)
                        {
                            context.Result = new RedirectToRouteResult("Dashboard", null);
                        }
                    }
