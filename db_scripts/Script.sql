...
BEGIN
    INSERT INTO MessageTemplate ([Name],[BccEmailAddresses],[Subject],[EmailAccountId],[Body],[IsActive],[DelayBeforeSend],[DelayPeriodId],[AttachedDownloadId],[AllowDirectReply],[LimitedToStores]) VALUES
    ('Customer.DeleteAccountRequest',NULL,'%Store.Name%. Account delete request.',1,'<p>  <a href="%Store.URL%">%Store.Name%</a> Account delete request </p> ',1,NULL,0,0,0,0)
...
BEGIN
	INSERT INTO MessageTemplate ([Name],[Subject],[EmailAccountId],[IsActive],[DelayPeriodId],[AttachedDownloadId],[AllowDirectReply],[LimitedToStores],[Body])
    select col,'%Store.Name% | Transaction Email', 1,1,0,0,1,0, '<p>  <a href="%Store.URL%">%Store.Name%</a> <br /><br />  Transaction Customer: %Customer.Email% <br />  Transaction Type: %Transaction.Type% <br />  Transaction Status: %Transaction.Status% <br />  Transaction Amount: %Transaction.Amount% <br />  Transaction Created: %Transaction.CreatedOn% <br />  </p>' from
