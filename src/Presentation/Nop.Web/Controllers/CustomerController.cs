...
namespace Nop.Web.Controllers
{
    ...
    public partial class CustomerController : BasePublicController
    {
        ...
        public virtual async Task<IActionResult> CustomerDocument(CustomerInfoModel model)
        {
            ...
            var formIdExtension = _nopFileProvider.GetFileExtension(model.FormId?.FileName ?? string.Empty).ToLower();
            var proofOfAddressExtension = _nopFileProvider.GetFileExtension(model.ProofOfAddress?.FileName ?? string.Empty).ToLower();
            var documentExtension = _nopFileProvider.GetFileExtension(model.Document?.FileName ?? string.Empty).ToLower();
            ...
            async Task<Download> InsertFormFiles(IFormFile file)
            {
                var download = new Download
                {
                    ...
                    Filename = _nopFileProvider.GetFileNameWithoutExtension(file.FileName),
                    Extension = _nopFileProvider.GetFileExtension(file.FileName),
                    IsNew = true
                };
